...
def add_message_metric(message_id: int, channel_id: int, author_id: int, commit: bool=True) -> MessageMetric:
  item = MessageMetric(message_id=str(message_id), timestamp=datetime.datetime.utcnow(), author_id=str(author_id), channel_id=str(channel_id))
  session.add(item)
  if commit:
    session.commit()
  return item
...
def get_author_of_last_message_metric(channel_id: int) -> Optional[int]:
  user_id = session.query(MessageMetric.author_id).filter(MessageMetric.channel_id == str(channel_id)).order_by(MessageMetric.timestamp.desc()).first()
  return int(user_id[0]) if user_id is not None else None

def add_message_content(channel_id: int, content: str):
  item = MessageContent(channel_id=str(channel_id), content=content)
  session.add(item)
  session.commit()
  return item
